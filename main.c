...
static ko_longopt_t long_options[] = {
    ...
    { "vcf-file",       ko_required_argument, 498 },
    { "parse-haplotype",ko_no_argument,       499 },
...
int main(int argc, char *argv[])
{
	...
	while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >= 0) {
		...
		else if (c == 498) vcf_with_variants = o.arg; // --vcf-file
  		else if (c == 499) ipt.flag |= MM_PARSE_HT;  // --parse-haplotype
	...
	if (argc == o.ind || fp_help == stdout) {
		...
		fprintf(fp_help, "    --vcf-file FILE	pass VCF FILE to modify index []\n");
